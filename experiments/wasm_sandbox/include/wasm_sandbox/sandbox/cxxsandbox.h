...
#include "wasm_sandbox/sandbox/internals.h"
...
namespace sandbox
{
  ...
  public:
    ...
    {
      CallFrame* callframe =
        (CallFrame*)lib.alloc_in_sandbox(sizeof(CallFrame));
      if (!callframe)
      {
        abort();
      }
      auto args2 = std::forward_as_tuple(args...);
      // Apparently arguments arrive in the wrong order in wasm.
      if (std::tuple_size<decltype(args2)>::value > 1)
        args2 = reverse_tuple(args2);
      callframe->args = args2;
      lib.send(vtable_index, callframe);
      if constexpr (!std::is_void_v<Ret>)
      {
        Ret r = callframe->ret;
        lib.free_in_sandbox(callframe);
        return r;
      }
      lib.free_in_sandbox(callframe);
    }
